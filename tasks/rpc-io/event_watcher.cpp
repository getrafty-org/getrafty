...
namespace getrafty::rpc::io {
    ...
    EventWatcher::EventWatcher() : EventWatcher(std::make_unique<ThreadPool>(1), ::epoll_wait) {
    };
    ...
    void EventWatcher::watch(const int fd, const WatchFlag flag, IWatchCallback *ch) {
    ...
    void EventWatcher::waitLoop() {
        ...
        if (const int n_fd = epollWaitFunc_(epoll_fd_, events, max_events, -1); n_fd == -1) {
            if (errno == EINTR) {
                // Retry on EINTR
            } else {
                LOG(ERROR) << "Fatal error in epoll_wait: " << strerror(errno);
