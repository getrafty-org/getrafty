...
public:
    MOCK_METHOD(void, onReadReadyMock /* NOLINT */, (int fd, const std::string& data));
    MOCK_METHOD(void, onWriteReadyMock /* NOLINT */, (int fd, const std::string& data));

    void onReadReady(const int fd) override {
        char buffer[1024];
        if (const ssize_t bytesRead = read(fd, buffer, sizeof(buffer)); bytesRead > 0) {
    ...
    void onWriteReady(const int fd) override {
        if (const ssize_t bytesWritten = write(fd, lastWritten.c_str(), lastWritten.size()); bytesWritten > 0) {
...
protected:
    ...
    static void setupPipe(int pipe_fd[2]) {
...
TEST_F(EventWatcherTest, WriteCallbackNotCalledWhenBufferFull) {
    ...
    ASSERT_EQ(write(test_fd[1], buffer_fill.c_str(), buffer_fill.size()), buffer_size);

    StrictMock<MockCallback> mock_callback;
    mock_callback.lastWritten = "y";

    EXPECT_CALL(mock_callback, onWriteReadyMock(test_fd[1], ::testing::_)).Times(0);
...
TEST_F(EventWatcherTest, WriteCallbackCalledOnceWhenBufferHasCapacity) {
    ...
    ASSERT_EQ(write(pipe_fd[1], first_half.c_str(), first_half.size()), first_half.size());

    MockCallback mock_callback;
    mock_callback.lastWritten = second_half;

    EXPECT_CALL(mock_callback, onWriteReadyMock(pipe_fd[1], ::testing::_)).Times(1);
    ...
    EXPECT_EQ(std::string(buffer.begin(), buffer.begin() + buffer_size / 2), first_half);
    EXPECT_EQ(std::string(buffer.begin() + buffer_size / 2, buffer.end()), second_half);
...
TEST_F(EventWatcherTest, NoCallbackIfFdNotReadyForRead) {
    ...
    EXPECT_CALL(mock_callback, onReadReadyMock(test_fd[0], ::testing::_)).Times(0);
...
TEST_F(EventWatcherTest, DuplicateWatchRequests) {
    ...
    EXPECT_CALL(mock_callback, onReadReadyMock(test_fd[0], ::testing::_)).Times(1);
...
TEST_F(EventWatcherTest, RetryOnEINTR) {
    ...
    auto alternating_epoll_mock = [&](int epfd, epoll_event *events, int maxevents, int timeout) {
